...
def g_LS_performance(base, amount, n_array, label):  #Linear Search
    ...
    #cria um novo mapa de cores
    colors1 = cm.get_cmap('Blues', 128)
    colors = np.vstack((colors1(np.linspace(0, 1, 128))))
    cmp = ListedColormap(colors)
...
def performance_comparison(a_array_length, label): #Performance Comparison
    ...
    length = len(a_array_length)*2
    scale = 5
    
    if(max_size/10 > 1):
        fig, axes = plt.subplots(1,length, constrained_layout=False, figsize=(length,max_size/scale))
    else:
        fig, axes = plt.subplots(1,length, constrained_layout=False, figsize=(length,3))

    
    i=0 #controla o 'a_array_length'
    search_type = 'BS'
    ...
    for current_ax in axes:
        ...
        current_ax.pcolormesh(np.row_stack(array),cmap=newcmp)
        current_ax.set_title(search_type + ' ' + str(a_array_length[i]))  
    ...
    #linhas
    if(len(a_array_length)>1):
        l_amount = 1/len(a_array_length) 
        l_position = l_amount
        for l in range(0, len(a_array_length)):
            line = lines.Line2D([l_position], [0,1], transform=fig.transFigure, figure=fig, color='#000000')
